...
import { getConfiguracion, updateEmpresa, syncDIAN as syncDIANApi } from '../services/api';
import { EMPRESA_DEFAULT, ROUTES } from '../utils/constants';
...
const Configuracion = () => {
  ...
  /** Estado de carga de operaciones contra la API */
  const [syncing, setSyncing] = useState(false);

  /** Cargar la configuracion de empresa y fiscal desde la API */
  useEffect(() => {
    let mounted = true;
    ...
    getConfiguracion()
      .then((res) => {
        ...
        setConfig((prev) => ({
          ...prev,
          empresa: res.configuracion.empresa,
          fiscal: res.configuracion.fiscal,
        }));
      })
      .catch((err) => {
  ...
  /** Guarda la configuracion de la empresa via la API */
  const saveEmpresa = useCallback(async () => {
    try {
      const res = await updateEmpresa(config.empresa);
      setConfig((prev) => ({ ...prev, empresa: res.empresa }));
      showToast('Datos de la empresa guardados', 'success');
    } catch (err) {
      showToast(err.message, 'error');
    }
  }, [config.empresa, showToast]);

  /** Sincroniza la configuracion con la DIAN via la API */
  const syncDIAN = useCallback(async () => {
    setSyncing(true);
    try {
      const res = await syncDIANApi();
      setConfig((prev) => ({
        ...prev,
        fiscal: { ...prev.fiscal, ultimaSync: res.fiscal.ultimaSync },
      }));
          ...
          <div className="config-section-card">
            ...
            <div className="roles-grid">
              ...
              <div className="role-card">
                ...
                <button className="btn-flat" onClick={() => navigate(ROUTES.USUARIOS)}>Gestionar</button>
              </div>
              {/* Rol Vendedores */}
              <div className="role-card">
            ...
            <div className="config-actions" style={{ marginTop: '20px' }}>
              <button className="btn-outline-accent" onClick={() => navigate(ROUTES.USUARIOS)}>
                <span className="material-icons">person_add</span>
                Gestionar Usuarios
              </button>
              <button className="btn-outline-accent" onClick={() => navigate(ROUTES.AUDITORIA)}>
